...
VBA マクロを Excel を起動せずに調査・解析するためのツール集。`.xlsm` / `.xlam` / `.xls` 内の VBA コードを直接バイナリパース層で取り出し、抽出・解析・比較・パスワード解除を行う。
...
| `Diff.bat` | 2 つの `.xlsm` の VBA コードを並列比較 | 不要 |
...
2 つの `.xlsm` を渡すと、両者の VBA コードをモジュール単位で並列表示比較する。
