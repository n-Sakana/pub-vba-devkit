...
`Sanitize.bat <path>` accepts `.xls`, `.xlsm`, and `.xlam` files. If a folder is passed, files are processed recursively. PowerShell callers can use `lib\Sanitize.ps1 -Path <path>`.
